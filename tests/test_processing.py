...
def test_filter_by_state_empty(transaction_data):
...
def test_sort_by_date(transaction_data, descending, expected_ids):
    result = sort_by_date(transaction_data, descending)
    assert [item['id'] for item in result] == expected_ids

def test_sort_by_date_invalid(transaction_data):
